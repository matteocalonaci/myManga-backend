...
@section('content')
    <div class="container" style="background-color: rgb(250, 0, 83); min-height: calc(100vh - 6.7rem);">
        <h1 class="text-center pt-4">Catalogo Genere</h1>

        <div class="d-flex flex-wrap justify-content-end align-items-center p-4">
            <button class="btn-add-editor btn btn-primary mt-3" type="button" onclick="window.location.href='{{ route('admin.genres.create') }}'">
                Aggiungi un nuovo genere
            </button>
        </div>

        <div class="search-bar mb-1 mt-2 d-flex justify-content-end mx-2 mx-md-4">
            <form method="GET" action="{{ route('admin.genres.index') }}" class="w-100">
                <div class="input-group">
                    <input type="text" name="search" class="form-control" placeholder="Cerca un genere..." value="{{ request('search') }}">
            ...
            @if ($genres->count() > 0)
                <table class="table table-sm table-striped table-bordered">
                    ...
                    <tbody id="editorTableBody">
                        @foreach ($genres as $genre)
                            <tr>
                                <td data-label="Nome">{{ $genre->name }}</td>
                                <td data-label="Azioni">
                                    <div class="d-flex flex-column flex-sm-row justify-content-between">
                                        <a href="{{ route('admin.genres.edit', $genre->id) }}" class="btn btn-warning flex-grow-1 mb-2 btn-custom">Modifica</a>
                                        <form action="{{ route('admin.genres.destroy', $genre->id) }}" method="POST" class="flex-grow-1"
                                            onsubmit="event.preventDefault(); Swal.fire({
                                                title: 'Elimina il genere?',
                                                text: 'Sei sicuro di eliminare il genere {{ $genre->name }}?',
                                                icon: 'warning',
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center mt-3">
                    <div class="px-3 px-md-5">
                        Mostra
                        <strong>{{ $genres->firstItem() }}</strong> a
                        <strong>{{ $genres->lastItem() }}</strong> di
                        <strong>{{ $genres->total() }}</strong> risultati
                    </div>
