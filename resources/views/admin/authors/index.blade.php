...
@section('content')
    <div class="container" style="background-color: rgb(250, 0, 83); min-height: calc(100vh - 6.7rem);">
        <h1 class="text-center pt-4">Catalogo Autori</h1>

        <div class="d-flex flex-wrap justify-content-end align-items-center p-4">
            <button class="btn-add-author btn btn-primary mt-3" type="button" onclick="window.location.href='{{ route('admin.authors.create') }}'">
                Aggiungi un nuovo autore
            </button>
        </div>

        <div class="search-bar mb-1 mt-2 d-flex justify-content-end mx-2 mx-md-4">
            <form method="GET" action="{{ route('admin.authors.index') }}" class="w-100">
                <div class="input-group">
                    <input type="text" name="search" class="form-control" placeholder="Cerca un autore..." value="{{ request('search') }}">
        ...
        <div class="table-responsive p-4">
            @if ($authors->count() > 0)
                <table class="table table-sm table-striped table-bordered">
                    ...
                    <tbody id="editorTableBody">
                        @foreach ($authors as $author)
                            <tr>
                                <td data-label="Nome">{{ $author->name }}</td>
                                <td data-label="Azioni">
                                    <div class="d-flex flex-column flex-sm-row justify-content-between">
                                        <a href="{{ route('admin.authors.edit', $author->id) }}" class="btn btn-warning flex-grow-1 mb-2 btn-custom">Modifica</a>
                                        <form action="{{ route('admin.authors.destroy', $author->id) }}" method="POST" class="flex-grow-1"
                                            onsubmit="event.preventDefault(); Swal.fire({
                                                title: 'Elimina l\'autore?',
                                                text: 'Sei sicuro di eliminare l\'autore {{ $author->name }}?',
                                                icon: 'warning',
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center mt-3">
                    <div class="px-3 px-md-5">
                        Mostra
                        <strong>{{ $authors->firstItem() }}</strong> a
                        <strong>{{ $authors->lastItem() }}</strong> di
                        <strong>{{ $authors->total() }}</strong> risultati
                    </div>
            ...
            @else
                <p class="text-center">Il catalogo degli author è attualmente vuoto.</p>
            @endif
        </div>
